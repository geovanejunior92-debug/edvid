 ...
 * bold-italic in `captions.accent` (orange #ff5200 by default, white on the
 * reference look), Poppins bold. Emphasis words appear solo; some
...
// The serif-italic accent line. Orange is the ORIGINAL look and stays the
// default, but the colour is data-driven (`captions.accent` in edit-data.json)
// since 2026-08-16: the reference this style is modelled on paints that word
// WHITE, and a hardcoded constant forced a fork of this file per project.
const ACCENT_DEFAULT = '#ff5200';
// Read here rather than from CAP below — LINE_STYLES is built before that block.
const SERIF_ACCENT =
  ((editData as {captions?: {accent?: string}}).captions ?? {}).accent || ACCENT_DEFAULT;
// undefined (not a fallback here) → PencilOutline keeps its own green default
const PENCIL_COLOR = ((editData as {captions?: {pencilColor?: string}}).captions ?? {}).pencilColor;
...
// 0=bold-italic, 1=regular(small), 2=serif-accent (orange by default,
// `captions.accent` overrides — white on the reference look), 3=bold
const LINE_STYLES: React.CSSProperties[] = [
  ...
  {fontFamily: PLAYFAIR, fontWeight: 900, fontStyle: 'italic', color: SERIF_ACCENT},
  ...
  if (cue.preset === 'STACK_MIXED') {
    ...
    inner = (
      <div style={{position: 'relative', display: 'inline-block'}}>
        {/* Green by default (PencilOutline's own default); `captions.pencilColor`
            overrides it — the all-white reference look needs a white ellipse. */}
        <PencilOutline progress={outlineProg} color={PENCIL_COLOR} />
        <span
          style={{
            ...WHITE_GRAD,
            fontFamily: POPPINS,
