...
import {loadFont as loadPlayfairHl} from '@remotion/google-fonts/PlayfairDisplay';
...
// Headline-only font choices (hook.font in edit-data.json). Kept separate from
// the module `fontFamily` above, which stays Poppins for the karaoke captions
// and the pop-in word graphic (lines using it directly) regardless of what the
// hook picks. Bebas Neue and Anton ship ONE weight (400) — loadFont errors on
// any other weight request, so HL_STYLES' per-style fontWeight (800/900) just
// rides whatever face got loaded; both faces already read as bold/condensed by
// design, so an unmatched CSS weight number is a no-op, not a visible gap.
const BEBAS = loadBebas('normal', {weights: ['400']}).fontFamily;
const ANTON = loadAnton('normal', {weights: ['400']}).fontFamily;
const PLAYFAIR_HL = loadPlayfairHl('italic', {weights: ['900']}).fontFamily;
const HL_FONTS: Record<string, string> = {
  poppins: fontFamily,
  bebas: BEBAS,
  anton: ANTON,
  playfair: PLAYFAIR_HL,
};
...
// `hook.font` picks the TYPEFACE independently of `hook.style` (see HL_FONTS
// above): "poppins" (default), "bebas", "anton", "playfair". Threaded through
// hlWidth/twoLines/fitHeadline so the two-line balance and the size-to-fit are
// measured in the actual chosen face, not always Poppins.
...
const hlWidth = (text: string, size: number, weight: number, fam: string) =>
  text
    ? measureText({text, fontFamily: fam, fontSize: size, fontWeight: weight, letterSpacing: '-1px'}).width
    : 0;
...
function twoLines(text: string, weights: [number, number], fam: string): [string, string] {
  ...
  for (let i = 1; i < words.length; i++) {
    ...
    const d = Math.abs(hlWidth(a, 100, weights[0], fam) - hlWidth(b, 100, weights[1], fam));
    if (d < bestDiff) {
      bestDiff = d;
      best = [a, b];
    }
...
function fitHeadline(lines: [string, string], s: HlStyle, fam: string): number {
  const widest = (size: number) =>
    Math.max(hlWidth(lines[0], size, s.weights[0], fam), hlWidth(lines[1], size, s.weights[1], fam));
...
const HookInner: React.FC<{totalFrames: number}> = ({totalFrames}) => {
  ...
  const size = fitHeadline(lines, {...S, cap, safeW: H.safeWidth ?? S.safeW}, hlFam);
    ...
    return (
      <AbsoluteFill style={{justifyContent: 'flex-start', alignItems: 'center', paddingTop: top}}>
        ...
        <div style={{opacity: op, translate: `0px ${y}px`, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 28}}>
          ...
          <div style={{background: '#232326', borderRadius: 24, padding: '28px 46px', textAlign: 'center', fontFamily: hlFam, fontWeight: 900, fontSize: size, color: '#fff', lineHeight: lh, letterSpacing: -1, textShadow: '0 4px 20px rgba(0,0,0,0.55)', boxShadow: '0 18px 50px rgba(0,0,0,0.45)'}}>
