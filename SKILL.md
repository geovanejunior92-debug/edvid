...
0. **Resolução da fonte é sagrada (2026-09-22).** O usuário grava em 4K60 no iPhone 18 Pro Max. Nenhuma etapa reduz a resolução: `render.py` mantém a da fonte por padrão, a Fase 2 renderiza o layout de 1080×1920 com `--scale` até o tamanho do `cut.mp4`, e o `--export-premium` segue a resolução do arquivo (45 Mbps HEVC em 4K). `--delivery-1080` só se ele pedir. fps também é o da fonte, short-form incluído: 60 continua 60 (o template Remotion ficou independente de fps em 2026-09-22; toda duração contada em quadros passa por `F()` em `assets/shortform/src/fps.ts`). `--shortform-fps` (30/24) só se ele pedir.
