...
0. **Resolução da fonte é sagrada (2026-09-22).** O usuário grava em 4K60 no iPhone 18 Pro Max. Nenhuma etapa reduz a resolução: `render.py` mantém a da fonte por padrão, a Fase 2 renderiza o layout de 1080×1920 com `--scale` até o tamanho do `cut.mp4`, e o `--export-premium` segue a resolução do arquivo (45 Mbps HEVC em 4K). `--delivery-1080` só se ele pedir. fps: short-form sai a 30 (o template Remotion tem animações contadas em quadros para 24/30); longform (`--keep-resolution`) e `--native-fps` mantêm os 60.
