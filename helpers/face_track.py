...
def detect(video: Path) -> tuple[float, int, int, list]:
    ...
    # YuNet (faces.py) primeiro: o Haar perde um rosto grande e inclinado no
    # 4K de câmera reduzido (0 de 32 amostras, 2026-09-01) e este acha. O Haar
    # fica de reserva quando o modelo ONNX não está no lugar.
    ...
    while True:
        ...
        box = detect_face(small) if detect_face else None
        if box is None:
            gray = cv2.cvtColor(small, cv2.COLOR_BGR2GRAY)
            faces = cascade.detectMultiScale(gray, scaleFactor=1.1, minNeighbors=5,
                                             minSize=(60, 60))
            if len(faces):
                box = tuple(int(v) for v in max(faces, key=lambda f: f[2] * f[3]))
        if box is not None:
            x, y, w, h = box
            cx = (x + w / 2) / DW
            cy = (y + h * 0.42) / (H * scale)
